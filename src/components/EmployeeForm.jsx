...
const EmployeeForm = () => {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case "payerQid":
        if (!val) return "";
        if (!/^\d{11}$/.test(val))
          return "Payer QID must be exactly 11 digits.";
        return "";
      ...
      case "payerIban":
        if (!val) return "";
        if (!/^[A-Z0-9]+$/.test(val))
          return "IBAN should be uppercase alphanumeric without spaces.";
        return "";
      case "qid":
        if (!val) return "Employee QID is required.";
        if (!/^\d{11}$/.test(val))
          return "Employee QID must be exactly 11 digits.";
        return "";
      case "name":
        if (!val) return "Employee name is required.";
        if (!/^[A-Za-z\s\-']+$/.test(val))
          return "Name may contain letters, spaces, hyphens or apostrophes.";
        return "";
      case "bank":
      ...
      case "salaryFrequency":
        if (!val) return "";
        if (!/^[A-Za-z]$/.test(val))
          return "Salary Frequency should be a single letter.";
        return "";
      default:
        return "";
    }
  ...
  const handleGenerate = async () => {
    ...
    const result = await window.electronAPI.saveSIFFile(
      payload,
      formData.payerEid,
      formData.payerBankShortName,
      sifContent.fileCreationDate,
      sifContent.fileCreationTime,
    );
    if (result.success) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-start justify-center py-10 px-4">
      <div className="relative w-full max-w-3xl flex flex-col gap-6">
        ...
        <SectionCard title="Employer Details" icon="🏢">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Payer EID (optional)" error={errors.payerEid}>
              <Input
                ...
                onChange={handleChange}
                error={errors.payerEid}
              />
            </Field>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Field
              label="Payer Bank Short Name"
              error={errors.payerBankShortName}
            >
              <Input
                name="payerBankShortName"
                placeholder="e.g. QNBA"
        ...
        <SectionCard title="Employee Details" icon="👤">
          <div className="grid grid-cols-2 gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Payment Type">
              <select
                ...
                className="
      h-10 w-full rounded-lg px-3.5 text-sm bg-slate-800/80 text-white
      border border-slate-700 transition-all duration-200 outline-none
      focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500
